/**
 * lib/cp/core.ts — Community Points mutation implementations.
 *
 * This module contains the actual DB logic for earnCP / burnCP / etc.
 * It does NOT import 'server-only' so that trusted server-side scripts
 * (e.g. scripts/grant-test-cp.ts) can import it directly via tsx without
 * hitting the Next.js client-component guard.
 *
 * Application code (Next.js routes, server actions, server components) must
 * import from lib/cp/index.ts, which re-exports everything from here PLUS
 * adds the 'server-only' guard. That way the guard is enforced on the
 * app-facing path while this file stays usable from plain Node scripts.
 *
 * Do not import this file from any client-side module.
 */
